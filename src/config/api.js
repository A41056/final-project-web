...
const BASKET_API_URL = import.meta.env.BASKET_API_URL || "http://localhost:6001";
const MEDIA_API_URL = import.meta.env.MEDIA_API_URL || "http://localhost:6010";
const REVIEW_API_URL = import.meta.env.REVIEW_API_URL || "http://localhost:6007";
...
const fetchFormDataWithAuth = async (url, formData) => {
...
export const userApi = {
    ...
    usePost: () => {
        ...
        return useMutation({
            mutationFn: ({ endpoint, data }) => fetchWithAuth(`${USER_API_URL}${endpoint}`, {
                method: "POST",
    ...
    useDelete: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: (endpoint) => fetchWithAuth(`${USER_API_URL}${endpoint}`, {
                method: "DELETE",
            }),
            onSuccess: () => queryClient.invalidateQueries({ queryKey: ["user"] }),
        });
    ...
    useResetPassword: () => useMutation({
        mutationFn: (data) => fetch(`${USER_API_URL}/reset-password`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
...
export const catalogApi = {
    useGet: (endpoint, params, options) => useQuery({
    ...
    usePost: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: ({ endpoint, data }) => fetchWithAuth(`${CATALOG_API_URL}${endpoint}`, {
                method: "POST",
                body: JSON.stringify(data),
            }),
    ...
    useDelete: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: (endpoint) => fetchWithAuth(`${CATALOG_API_URL}${endpoint}`, {
                method: "DELETE",
            }),
            onSuccess: () => queryClient.invalidateQueries({ queryKey: ["catalog"] }),
        });
...
export const orderApi = {
    ...
    usePost: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: ({ endpoint, data }) => fetchWithAuth(`${ORDER_API_URL}${endpoint}`, {
                method: "POST",
                body: JSON.stringify(data),
            }),
    ...
    useDelete: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: (endpoint) => fetchWithAuth(`${ORDER_API_URL}${endpoint}`, {
                method: "DELETE",
            }),
            onSuccess: () => queryClient.invalidateQueries({ queryKey: ["order"] }),
        });
...
export const basketApi = {
    useGet: (endpoint, options) => useQuery({
    ...
    usePost: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: ({ endpoint, data }) => fetchWithAuth(`${BASKET_API_URL}${endpoint}`, {
                method: "POST",
                body: JSON.stringify(data),
            }),
    ...
    useDelete: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: (endpoint) => fetchWithAuth(`${BASKET_API_URL}${endpoint}`, {
                method: "DELETE",
            }),
            onSuccess: () => queryClient.invalidateQueries({ queryKey: ["basket"] }),
        });
...
export const mediaApi = {
    useGetFileTypes: (endpoint) => useQuery({
    ...
    useUploadFile: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: (formData) => fetchFormDataWithAuth(`${MEDIA_API_URL}/files`, formData),
    ...
    useDeleteFile: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: (fileName) => deleteWithAuth(`${MEDIA_API_URL}/files/${fileName}`),
            onSuccess: () => queryClient.invalidateQueries({ queryKey: ["media"] }),
        });
    },
};
export const reviewApi = {
    useGetReviewsByProductId: (productId, pagination) => useQuery({
    ...
    usePost: () => {
        const queryClient = useQueryClient();
        return useMutation({
            mutationFn: (data) => fetch(`${REVIEW_API_URL}/reviews`, {
                method: "POST",
